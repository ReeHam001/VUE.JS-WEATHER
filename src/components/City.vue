<template>
  <div @click="goToWeather" class="city">
    <i v-if="edit" @click="removeCity(this.city.city)" class="far fa-trash-alt edit" ref="edit"></i>
...
export default {
  ...
  computed: {
    imageSrc() {
      return new URL(`../../public/conditions/${this.city.currentWeather.weather[0].icon}.svg`, import.meta.url).href;
    },
    videoSrc() {
      return new URL(`../../public/videos/${this.city.currentWeather.weather[0].icon}.mp4`, import.meta.url).href;
    }
  },
  methods: {
    async removeCity() { 
      ...
      docs.forEach((doc) => {
       ...
       deleteDoc(doc(db, "cities", this.id));
